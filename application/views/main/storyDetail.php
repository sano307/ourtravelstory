<!--
<div class="mdl-grid">
    <div class="mdl-cell mdl-cell--12-col" style="border: 1px solid red; height: 12%;">
        ...
        <div style="padding-left:20px; background-color:#263238;">
                <div style="padding-top:10px; padding-bottom:10px;">
                    <?php
                        $data = 1;
                        foreach($StoryPlace as $row){
                            if($data == $row->StoryPlaceDateNumber) { ?>
                                <a href="/Storydetail/storyDetail/<?=$row->StoryIdx ?>/<?=$row->StoryPlaceDateNumber ?>/<?= $row->StoryPlaceIdx ?>">_<span class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button-accent">
...
<div class="mdl-grid">
    <div class="mdl-cell mdl-cell--2-col" style="border: 1px solid red; height: 70%;">
        <ul>
        <?php foreach($StoryPlaceName as $place) { ?>
            <li><?= $place->placeName; ?></li>
        <?php } ?>
    </div>
    <div class="mdl-cell mdl-cell--10-col" style="border: 1px solid red; height: 70%;">
    ...
    <div style="padding-top:10px; padding-bottom:10px;">
        <?php $data = 1; foreach($StoryPlace as $row){
            if($data == $row->StoryPlaceDateNumber){?>
                ...
                <b><?php echo $row->StoryPlaceDateNumber ?>日</b>
            </span></a>
                <?php $data++; }} ?>
    </div>
                    ...
                    <div class="location_info">
                        <div class="demo-card-square mdl-card mdl-shadow--2dp" style="width:100%; height:100%;">
                            ...
                            <div class="mdl-card__actions mdl-card--border"  style="margin-bottom:0.5%; margin-top:0%; padding-top:0%; ">
                                <div class="material-icons mdl-badge mdl-badge--overlap" id="footprint" data-badge="<?php echo $placeinfo ->placeFootprint; ?>"  style="margin-bottom:0%; margin-top:3%; padding-right:1%; padding-top:0%; float:right; width:28px; color:#3c90be; font-size:33px;">pets</div>
                              <?php }?>
                                <!-- 장소 좋아요 추가-->
                                <?php if($StoryPlaceGoodCount){ ?>
                                    <a><div class="material-icons mdl-badge mdl-badge--overlap" id="good_check" data-badge="<?php echo $StoryPlaceGoodCount[0]->StoryPlaceGoodCount; ?>" style="margin-bottom:0%; margin-top:3%; padding-top:0%; padding-right:1%; margin-left:1%; float:right; width:28px; color:hotpink; font-size:33px;">favorite</div></a>
                                <?php }else{ ?>
                                    <a><div class="material-icons mdl-badge mdl-badge--overlap" id="good_check" data-badge="<?php echo $StoryPlaceGoodCount[0]->StoryPlaceGoodCount; ?>" style="margin-bottom:0%; margin-top:3%; padding-top:0%; padding-right:1%; margin-left:1%; float:right; width:28px; color:gray; font-size:33px; ">favorite_border</div></a>
                                <?php } ?>
                                <script>
                                    var good = 0;
                                    // 장소별 좋아요
...
<script>
    ...
    $('.grid-item-content').on('click', '.show_image_modify_dialog', function() {
        ...
        var imageUrl = "/images/" + temp_class_arr[2];

        var temp_element = '<table><tr><td>';
        temp_element += '<h3 style="margin-left:7px;">イメージ修正</h3>';
        temp_element += '<div id="colorpicker"></div>';
        temp_element += '<input type="text" id="color" name="color" style="margin-bottom:10px; margin-top:10px; margin-left:7px; " /><br />';
        temp_element += '<p style="margin-left:35px; ">Font size : <select id="brushThick" style="width: 60px; "><option value="1">1</option><option value="3">3</option><option value="5">5</option>';
        ...
        temp_element += '</select></p>';
        temp_element += '<button id="btnUndo" class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--accent"><-</button>';
        temp_element += '<button id="btnDelete" class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--accent" >Clean</button>';
        temp_element += '<button id="btnRedo" class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--accent">-></button>';
        temp_element += '<a id="test" href="" download><button class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--accent" style="float:right; margin-right:57px; margin-top:20px;">イメージ貯藏</button></a>';
        temp_element += '<br /></td><td><canvas id="myCanvas" width="740" height="500" style="border:2px solid black"></canvas></td></tr></table>';
    ...
    function imageOnKeyDown(image_id,story_id)
    {
        if(event.keyCode == 13)  {
            ...
            $.ajax({
                ...
                data: {user_id:login_user_idx, reply_content:input_reply, image_id: image_id, story_id: story_id },
                dataType: 'jsonp',
                success: function (data) {
    ...
    function placeOnKeyDown(place_id)
    {
        if(event.keyCode == 13)  {
            ...
            $.ajax({
                ...
                data: {user_id:login_user_idx, reply_content:input_reply, place_id: place_id },
                dataType: 'jsonp',
                success: function (data) {
                    $('#placereply').empty();
                    for(var count = 0; count < data.result.length; count++) {
